...
batch 内对**有符号梯度**求均值，再让每个神经元除以它自己的最终时间步梯度绝对值，
并只画 LIF 与 LS 两个面板。横轴是时间，纵轴是按模型注册顺序拼接后均匀采样的隐含神经元；两幅图共用
...
这里比较的是 `g_t / |g_T|`：如果两个神经元的最终梯度分别相差 10 倍，但时间衰减比例相同，
它们会显示相同的传播形状。因此图不会把“末端起始梯度更大”误当成“传播得更远”。最终梯度
数值为 0 的行没有可靠参照，会整行置 0，避免除以极小数后被虚假放大。0 仍严格对应白色。
