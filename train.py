...
def main():
    ...
    parser.add_argument('-neuron_model', type=str, default='LIF', help='neuron model: LIF (vanilla), SRLIF (Synaptic Release LIF), SRLIF2 (membrane-only synaptic release LIF), SCRLIF (Spike-Cause Reset LIF), SCRLIFV2, HALIF, ZELIF, IDISILIF, newLIF (adaptive tau), newLIFTauDep (tau-dependent adaptive tau), newCLIF (CLIF + tau-dependent adaptive tau), DTLIF (direct rho update), DGN, LIFDGN, LIFDGN2, LIFDGN3, LSLIF, LSLIF2, LSLIF3, LSLIF4, LSCLIF, LSPLIF, RCMLIF, TLIF, QKVLIF, CLIF, PLIF, relu')
    ...
    parser.add_argument('-release_threshold_init', type=float, default=0.0, help='for SRLIF/SRLIF2 only: initial shared synaptic release threshold')
    ...
    parser.add_argument('-rcm_eta', type=float, default=1.0, help='for RCMLIF only: reset-loss memory update coefficient eta')
    parser.add_argument('-rcm_beta', type=float, default=1.0, help='for RCMLIF only: reset-loss compensation coefficient beta')
    parser.add_argument('-rcm_learn_eta', action='store_true', help='for RCMLIF only: make eta learnable')
    ...
    parser.add_argument('-dgn_dt', type=float, default=1.0, help='for DGN only: simulation step size Δt')
    parser.add_argument('-dgn_gl', type=float, default=0.0, help='for DGN only: fixed leak conductance g_l')
    parser.add_argument('-dgn_c_init', type=float, default=0.01, help='for DGN only: init value for conductance-path parameter C')
    parser.add_argument('-dgn_w_init', type=float, default=0.01, help='for DGN only: init value for current-path parameter W')
    parser.add_argument('-dgn_learn_c', dest='dgn_learn_c', action='store_true', help='for DGN only: make C trainable (default: enabled)')
    parser.add_argument('-dgn_fix_c', dest='dgn_learn_c', action='store_false', help='for DGN only: freeze C (disable learning)')
    parser.add_argument('-dgn_learn_w', dest='dgn_learn_w', action='store_true', help='for DGN only: make W trainable (default: enabled)')
    parser.add_argument('-dgn_fix_w', dest='dgn_learn_w', action='store_false', help='for DGN only: freeze W (disable learning)')
    ...
    parser.add_argument('-lifdgn_learn_g0', dest='lifdgn_learn_g0', action='store_true', help='for LIFDGN only: make g0 trainable (default: enabled)')
    parser.add_argument('-lifdgn_fix_g0', dest='lifdgn_learn_g0', action='store_false', help='for LIFDGN only: freeze g0 (disable learning)')
    ...
    if args.neuron_model == 'LIF':
    ...
    elif args.neuron_model == 'SRLIF2':
        neuron_model = neuron.SRLIF2Neuron
        ...
        neuron_model = neuron.LSPLIFNeuron
    ...
    neuron_kwargs = dict(
        ...
        rcm_learn_eta=args.rcm_learn_eta,
    ...
    if args.neuron_model in ['SRLIF', 'SRLIF2']:
        run_name_parts.extend([
            f'释放阈值初值{args.release_threshold_init}',
        ])
        if args.neuron_model == 'SRLIF':
            run_name_parts.append(f'释放门控比例{args.srlif_release_ratio}')
    if args.neuron_model in ['newLIF', 'newLIFTauDep', 'newCLIF']:
        ...
        c_can_learn = '是' if args.dgn_learn_c else '否'
        w_can_learn = '是' if args.dgn_learn_w else '否'
        run_name_parts.extend([
            f'dgn_dt{args.dgn_dt}',
            f'dgn_gl{args.dgn_gl}',
            f'dgn_Cinit{args.dgn_c_init}',
            f'dgn_Winit{args.dgn_w_init}',
        ...
        eta_can_learn = '是' if args.rcm_learn_eta else '否'
        beta_can_learn = '是' if args.rcm_learn_beta else '否'
        run_name_parts.extend([
            f'rcm_reset{args.rcm_reset}',
            f'rcm_vreset{args.rcm_v_reset}',
            f'rcm_lambda{args.rcm_lambda}',
            f'rcm_eta{args.rcm_eta}',
            f'rcm_beta{args.rcm_beta}',
            f'rcm_eta可学习{eta_can_learn}',
    ...
    if is_main_process:
        ...
        with open(metrics_csv_path, 'w', encoding='utf-8', newline='') as csv_f:
            writer_csv = csv.writer(csv_f)
            writer_csv.writerow(['epoch', 'train_loss', 'train_acc', 'test_loss', 'test_acc', 'max_test_acc', 'lr', 'epoch_time_sec', 'gpu_mem_gb', 'test_spike_rate_global', 'total_params_m', 'trainable_params_m'])
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        with open(os.path.join(out_dir, 'args.txt'), 'a+', encoding='utf-8') as args_txt:
            ...
            args_txt.write(f'lr={current_lr}, total_params_m={total_params / 1e6}, trainable_params_m={trainable_params / 1e6}' + "\n")

        if is_main_process:
            with open(os.path.join(out_dir, 'metrics.csv'), 'a', encoding='utf-8', newline='') as csv_f:
                writer_csv = csv.writer(csv_f)
                writer_csv.writerow([epoch, train_loss, train_acc, test_loss, test_acc, max_test_acc, current_lr, total_time, gpu_mem_gb, global_spike_rate, total_params / 1e6, trainable_params / 1e6])

            if epoch == args.epochs - 1:
                with open(os.path.join(out_dir, 'run_summary.json'), 'r', encoding='utf-8') as f:
                    run_summary = json.load(f)
                run_summary.update({
                    'best_test_acc': float(max_test_acc),
                    'final_test_acc': float(test_acc),
                    'final_test_loss': float(test_loss),
                    'final_test_spike_rate_global': float(global_spike_rate),
                })
                with open(os.path.join(out_dir, 'run_summary.json'), 'w', encoding='utf-8') as f:
                    json.dump(run_summary, f, ensure_ascii=False, indent=2)
