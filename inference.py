...
def main():
    ...
    parser.add_argument('-tau_alpha_up', type=float, default=0.02, help='for newLIF only: alpha when no spike (drives more leakage)')
    parser.add_argument('-tau_alpha_down', type=float, default=0.02, help='for newLIF only: alpha when spike (drives more retention)')
    ...
    if args.neuron_model in ['LSLIF', 'TLIF']:
        ...
        out_dir += f'_aux{args.lslif2_aux_mode}'
        if args.lslif2_aux_mode == 'scaled_avg':
            out_dir += f'_hw{args.history_weight}_he{args.history_eps}_hm{args.history_mode}_hlw{int(args.history_learn_weight)}'
    elif args.neuron_model == 'RCMLIF':
        out_dir += (
            f'_rr{args.rcm_reset}_rv{args.rcm_v_reset}_rl{args.rcm_lambda}'
        ...
        out_dir += (
            ...
            f'_la{int(args.dtlif_learn_a)}_lb{int(args.dtlif_learn_b)}'
            f'_llo{args.dtlif_lambda_lo}_lhi{args.dtlif_lambda_hi}'
        )
    elif args.neuron_model == 'DGN':
        out_dir += (
            f'_dt{args.dgn_dt}_gl{args.dgn_gl}'
            f'_c{args.dgn_c_init}_w{args.dgn_w_init}'
        ...
        out_dir += (
            ...
            f'_lg0{int(args.lifdgn_learn_g0)}_lc{int(args.lifdgn_learn_c)}'
            f'_gmax{args.lifdgn_g_max}'
        )
